...
import { addMultipleToCart} from '../redux/actions'; 
...
const DATA = [{id:1,name:"Jad Taous",subtitle:"Favourites - 1 item",active:true},{id:2,name:"Youssef Taous",subtitle:"1 item",active:false},{id:3,name:"Your List",subtitle:"No items added yet!",active:false}];
  ...
  const Item = ({ item }) => (
    ...
    <TouchableOpacity onPress={()=>{setSelectedWishlist(item);setModalVisible(true)}}><View  style={[styles.item,{backgroundColor:item.active?"#F2F8FD":"white",flexDirection:"row"}]}>
      ...
      <Text style={styles.name}>{item.product_details?item.product_details.length:0} {t('common_items_lbl')}</Text>
      </View>
  ...
  return (
    ...
    <TouchableOpacity onPress={()=>{setWishlistName('');setWishlistId(0);setModalVisibleAdd(true)}} style={{height:35,width:"90%"}}><View style={{justifyContent:"center",alignItems:"center",backgroundColor:Colors.drawerHeaderBackground,height:35,width:"100%",borderRadius:20}}>
    <Text style={{color:"white",fontFamily:FONT.RobotoBold}}>Create a new list</Text>
   ...
   {isWishlistLoading? <View style={{marginVertical:6,marginHorizontal:20,justifyContent:"center",alignItems:"center",flex:1}}><ActivityIndicator size={"small"} color={Colors.greenBtnColor} /></View>:datawishlist.length > 0 ? <View style={{marginVertical:6,marginHorizontal:20}}>
      ...
      <Text style={{fontSize:14,color:Colors.drawerHeaderBackground,fontFamily:FONT.RobotoBold}}>{t('wishlists_yettocreate_lbl')}</Text>
